...
with open("d:\\GAS WMS Mini\\ViewFulfillment.html", "r", encoding="utf-8") as f:
    content = f.read()
...
new_js = r"""
  ...
  async function muatDataFulfillment(force) {
    ...
    try {
        const res = await fetch(`${SUPABASE_URL}/rest/v1/picking_list?order=created_at.desc`, {
  ...
  async function handleProcessFulfillment(force = false) {
    ...
    try {
        // Load Realtime Stock for Location Allocation
        const stockRes = await fetch(`${SUPABASE_URL}/rest/v1/view_stok_realtime?select=sku,lokasi,sisa_stok&sisa_stok=gt.0&area=eq.Warehouse`, {
            headers: { 'apikey': SUPABASE_ANON_KEY, 'Authorization': 'Bearer ' + SUPABASE_ANON_KEY }
        });
        ...
        stockData.forEach(st => {
            ...
            stockMap[s].push({lokasi: st.lokasi, qty: st.sisa_stok});
        });
        ...
        if (!insRes.ok) throw new Error("Gagal menyimpan ke Supabase");
...
new_content = re.sub(r'<script>\s*let FULFILLMENT_ON_PROCESS = \[\];.*?</script>', lambda m: '<script>\n' + new_js + '\n</script>', content, flags=re.DOTALL)

with open("d:\\GAS WMS Mini\\ViewFulfillment.html", "w", encoding="utf-8") as f:
    f.write(new_content)

print("Patch Fulfillment successful!")
